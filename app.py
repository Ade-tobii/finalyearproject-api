...
import numpy as np
from agro_ai import analyze_soil_data
...
def receive_sensor_data():
    try:
        ...
        response = supabase_client.table('sensor_data').insert(soil_data).execute()
        inserted_id = response.data[0]['id']
        ...
        if recommendation and severity:
            with app.test_request_context():
                rec_response = app.test_client().post(
                    '/api/recommendations',
                    json={"recommendation": recommendation, "severity": severity}
                )
                recommendation_sent = rec_response.status_code == 201
                logger.info("Recommendation forwarded internally")
